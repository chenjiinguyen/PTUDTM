...
namespace DLL.Service
{
    public class EpisodeService
    {
        ...
        public List<episode> GetUncensorByIdBook(long id)
        {
            List<long> listEpi = db.moderation_episode.Where(x => x.review == false && x.episode.idbook == id).Select(x => x.idepisode).ToList();
            return db.episodes.Where(x=> listEpi.Contains(x.idepisode) && x.idbook == id).ToList();
        }
